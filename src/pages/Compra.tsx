...
export default function Compra() {
  ...
  const handleSubmit = (e) => {
    ...
    // Validación básica
    if (!form.name || !form.email || (paymentMethod !== 'paypal' && (!form.cardNumber || !form.cardName || !form.expiry || !form.cvv))) {
    ...
    <div className="flex items-center justify-center min-h-screen p-4">
      <div className="w-full max-w-xl p-8 bg-gray-800 rounded shadow-lg">
        ...
        <div className="flex justify-center gap-4 mb-6">
          {[
            ...
            <button
              ...
              onClick={() => handlePaymentSelect(p.method)}
            >
              {p.icon}
            </button>
          ))}
        ...
        <form onSubmit={handleSubmit} autoComplete="off" className="flex flex-col gap-4">
          ...
          <textarea name="message" value={form.message} onChange={handleChange} placeholder="Detalles adicionales del pedido" rows="4" className="p-3 rounded bg-gray-700 text-white" />

          {paymentMethod !== 'paypal' && (
            <>
              <input type="text" name="cardNumber" value={form.cardNumber} onChange={handleChange} placeholder="Número de tarjeta" required pattern="\d{13,16}" inputMode="numeric" title="Ingrese un número de tarjeta válido" className="p-3 rounded bg-gray-700 text-white" />
              ...
              <input type="text" name="expiry" value={form.expiry} onChange={handleChange} placeholder="MM/AA" required pattern="(0[1-9]|1[0-2])\/\d{2}" title="Formato MM/AA" className="p-3 rounded bg-gray-700 text-white" />
              <input type="text" name="cvv" value={form.cvv} onChange={handleChange} placeholder="CVV" required pattern="\d{3,4}" inputMode="numeric" title="CVV de 3 o 4 dígitos" className="p-3 rounded bg-gray-700 text-white" />
            </>
          )}

          <button type="submit" disabled={loading} className="bg-blue-900 text-white px-6 py-3 rounded-full hover:bg-blue-700 transition mt-2">
